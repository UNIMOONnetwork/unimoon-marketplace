...
fn legacy_validation(
    ...
    if participation_printing_account.amount == 0 {
        return Err(UniMoonError::ParticipationPrintingEmpty.into());
    }
...
fn v2_validation<'a>(
    ...
    if destination_amount != 1 {
        return Err(UniMoonError::ProvidedAccountDoesNotContainOneToken.into());
    }
...
fn charge_for_participation<'a>(
    ...
    if bidder_token.amount.saturating_sub(price) < 0 as u64 {
        return Err(UniMoonError::NotEnoughBalanceForParticipation.into());
    }
