...
pub fn assert_common_checks(
    ...
    if auction.authority != *auction_manager_info.key && auction.authority != *authority_info.key {
        return Err(UniMoonError::AuctionAuthorityMismatch.into());
    }
    ...
    if auction.state != AuctionState::Created {
        return Err(UniMoonError::AuctionMustBeCreated.into());
    }
    ...
    if auction.token_mint != accept_payment.mint {
        return Err(UniMoonError::AuctionAcceptPaymentMintMismatch.into());
    }

    if accept_payment.owner != *auction_manager_info.key {
        return Err(UniMoonError::AcceptPaymentOwnerMismatch.into());
    }
    ...
    if vault.state != VaultState::Combined {
        return Err(UniMoonError::VaultNotCombined.into());
    }

    if vault.token_type_count == 0 {
        return Err(UniMoonError::VaultCannotEmpty.into());
    }
